...
| **Ruling 3** | **FOUR SECTIONS, PLACED BY THE PERSON** (Cory 2026-09-15, paste 133; **the ORDER amended by his 2026-09-20 review, paste 137 S1**) — Morning routine · Night routine · **Weekly routine** · Standards, in that order, and a standard sits where HE put it (`habits.section`), never where the clock would put it. This supersedes DEC-171's last clause ("Timed standards group under TIMED, then ANYTIME, then WEEKLY"): those three were computed from cadence and the planned time. A row with no section yet shows where it showed before — weekly → Weekly, a planned time → Morning, no time → Standards, the Sabbath → Night — and the SAME rule is written three times over, in `HT29SEC.sectionOf`, in the vault copier (`tools/copiers/_ht.py`) and in the sender (`core.js`); `golden_ht29` S2 and `golden_ht30` S1 hold them together. **The ORDER is declared once, in `HT29SEC.ORDER`**, and `HT29MD.SECTIONS` reads the same list. Inside Morning and Night the planned time orders the rows, as TIMED always did; inside Standards and Weekly the drag order does. **The definition of done is `habits.notes`, relabelled "Done when" — no new column.** HT-30 S3.8 takes that field OFF the edit sheet (`HT30_SHEET_NOTES = false`): the textarea is not rendered and `notes` is therefore never written, so every definition already saved survives untouched (R70.138). It rides the name on a desktop hover as before. |
